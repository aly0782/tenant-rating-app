...
const WRAPPERS_JSON = path.join(DAPP_DIR, "src/config/wrappers.json");
const CONFIG_JSON = path.join(DAPP_DIR, "src/config/config.json");
...
export const scaffold: Runner = async (args: Args, ui: UIProvider) => {
  ...
  await fs.mkdir(path.join(DAPP_DIR, "src/config"), { recursive: true });
